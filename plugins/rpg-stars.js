...
let handler = async (m, {conn, usedPrefix}) => {
    ...
    let img = await (await fetch(`https://qu.ax/mSayB.jpg`)).buffer()
    let txt = ` –  *S T A R S  -  U S E R*\n\n`
        txt += `┌  ✩  *Nombre* : ${user.name}\n`
        txt += `│  ✩  *NanoCoins* : ${toNum(user.limit)} ( *${user.limit}* )\n`
        txt += `│  ✩  *Bank* : ${toNum(user.bank)} ( *${user.bank}* )\n`
        txt += `└  ✩  *XP* : ${toNum(user.exp)} ( *${user.exp}* )`
    let mentionedJid = [who]
        
await conn.sendFile(m.chat, img, 'https://qu.ax/mSayB.jpg', txt, m, null, rcanal)
}
